...
volatile byte buzzer_state = LOW;
...
void setup()
{
  ...
  // Gaz interrupt initialization
  pinMode(interruptPin,INPUT);
  attachInterrupt(digitalPinToInterrupt(interruptPin),gaz_alert,CHANGE);

  // Timer interrupt initialization
  MsTimer2::set(500, InterruptTimer2); // period 500ms
  MsTimer2::start(); // launch timer2
...
void loop()
{
    ...
    byte data[3];
    if (alert_state == 0){
      if (buzzer_state == 1) { // Buzzer ON
        tone(buzzerPin,1000);
      }
      else if (buzzer_state == 0) { // Buzzer OFF
        noTone(buzzerPin);
      }
      data[0] = 0x00; // Sending no alert
    }
    else if (alert_state == 1) {
      data[0] = 0x01; // Sending alert
    }
    data[1] = gaz >> 8;
    data[2] = gaz & 0xFF;
...
void gaz_alert() { // Interrupt fct on gaz sensor
  alert_state =  !alert_state;
}

void InterruptTimer2() { // Interrupt fction on timer
